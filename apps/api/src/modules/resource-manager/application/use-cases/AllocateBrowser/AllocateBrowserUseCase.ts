...
export interface AllocateBrowserCommand {
  executionId: string;
  durationSeconds: number;
}

export class AllocateBrowserUseCase {
  constructor(
  ...
  public async execute(command: AllocateBrowserCommand): Promise<string> {
    const pool = await this.poolRepo.findByType(ResourceType.Browser);
    if (!pool) throw new Error('Browser pool not initialized');
    ...
    const lease = ResourceLease.create(
      ResourceId.create(`browser-res-${rawId}`),
      command.executionId,
