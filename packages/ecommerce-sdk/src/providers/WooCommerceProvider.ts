...
class WooCommerceSession implements IECommerceSession {
  sessionId = `woo-session-${Date.now()}`;
...
export class WooCommerceProvider implements IECommerceProvider {
  async authenticate(credentials: Record<string, string>): Promise<IECommerceSession> {
    console.log(`[WooCommerceProvider] Authenticating via REST API Consumer Key/Secret...`);
